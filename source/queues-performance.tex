...
\begin{table}[htb]
  ...
  \caption[Idle workers]{Number of times workers run out of work and become idle, and get woken up again}
...
\ref{tab:queues-performance-idempotent-fifo}). In the \emph{Idempotent
  Work-Stealing Deque} there is less contention between
...
\begin{table}[!htb]
  ...
  \caption[Idempotent Work-Stealing FIFO Queue]{Comparing take and steal attempts and failures of the \emph{Idempotent Work-Stealing Deque} with the \emph{Idempotent Work-Stealing FIFO Queue}}
...
\lstinline!steal()! remove tasks at the tail too. As \lstinline!put!,
\lstinline!take()! and \lstinline!steal()! work on the same end of the
queue, they have to deal with increased contention. This especially
shows in the performance of the \emph{Series} and \emph{MonteCarlo}
benchmarks with an extremely high number of steal attempts and
failures compared to the \emph{Idempotent Work-Stealing Deque} (Table
...
\begin{table}[!htb]
  ...
  \caption[Idempotent Work-Stealing LIFO Queue]{Comparing take and steal attempts and failures of the \emph{Idempotent Work-Stealing Deque} with the \emph{Idempotent Work-Stealing LIFO Queue}}
...
\ref{chap:experimental-setup}). Saha et al. \cite{Saha2007} state that
there is no noticeable difference between the speedup of work-stealing
and a global shared work queue when not using more than 8 cores. If
this is the case, then the optimizations we did in our work-stealing
queue implementations most likely wont have a significant effect
either.
...
\lstinline!LinkedBlockingDeque! from \lstinline!java.util.concurrent!
in a stack-like manner: Items are added with \lstinline!addLast()! at
the end of the queue and take with \lstinline!pollLast()! from the end
of the queue. For comparison we added another work-stealing
implementation changing the original intervals scheduler to use
\lstinline!LinkedBlockingDeque!.
...
\begin{figure}[!ht]
  \centering
  \subfloat[Intel Core2 Duo: 2 threads, 2 workers]{
    \includegraphics[width=0.8\linewidth]{queues-performance/marvin-shared-queue}
    \label{fig:queues-performance-marvin-shared-queue}
  }
  \\
  \subfloat[Intel Nehalem: 8 threads, 8 workers]{
    \includegraphics[width=0.8\linewidth]{queues-performance/mafushi-shared-queue}
