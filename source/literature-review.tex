...
\todo{Integrate references listed into other sections, for example
  ``Related Work'' and remove this chapter}
...
\item[\checkmark] Erlang Programming Language, Official Website
  \cite{Erlang2010}
\item[\checkmark] OpenMP Application Program Interface, Version 3.0
  \cite{OpenMP2008}
\item[\checkmark] MPI: A Message-Passing Interface Standard, Version
  2.2 \cite{MPI2009}
\item[\checkmark] The Java memory model \cite{Manson2005}
\item[\checkmark] Design of a separable transition-diagram compiler
  \cite{Conway1963}
\item[\checkmark] The discoveries of continuations \cite{Reynolds1993}
\item[\checkmark] MULTILISP: a language for concurrent symbolic
  computation \cite{Halstead1985}
\item[\checkmark] Quasi-static scheduling for safe futures
  \cite{Navabi2008}
\item[\checkmark] The JCilk language for multithreaded computing
  \cite{Danaher2005}
\item[\checkmark] The design, implementation, and evaluation of Jade
  \cite{Rinard1998}
\item[\checkmark] JSR 166: Concurrency Utilities \cite{Lea2004}
\item[\checkmark] Managing Concurrency with NSOperation
  \cite{Apple2008}
\end{itemize}
